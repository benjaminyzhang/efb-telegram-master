...
from ehforwarderbot.status import (
    ChatUpdates,
    MemberUpdates,
    MessageRemoval,
...
def send_message_test(chat_id, text):
...
def send_to_cubox(type, content, title=None, description=None, tags=None, folder=None):
    payload = {
        "type": type,
        "content": content,
        "title": title,
    ...
    # 假设API的URL是 http://api.cubox.com/v1/item，你可能需要将其替换为实际的URL
...
class SlaveMessageProcessor(LocaleMixin):
    ...
    def is_silent(self, msg: Message) -> Optional[bool]:
        ...
        if isinstance(msg.author, SelfChatMember):
            ...
            msg.chat.notification == ChatNotificationState.MENTIONS
            and (not msg.substitutions or not msg.substitutions.is_mentioned)
        ):
            ...
            if muted_on_slave == "silent":
                ...
                self.logger.debug(
                    "[%s] Message is muted due to slave channel settings.", xid
                )
                return None
        return False
    ...
    def send_message(self, msg: Message) -> Message:
        ...
        try:
            ...
            if silent is None:
                self.logger.debug(
                    "[%s] Message is not delivered per silent settings.", xid
                )
                return msg
    ...
    def dispatch_message(
        ...
        if isinstance(msg.target, Message):
            ...
            if not log:
                ...
                self.logger.debug(
                    "[%s] Target message has database entry: %s.", msg.uid, log
                )
        ...
        if msg.type == MsgType.Text:
            tg_msg = self.slave_message_text(
                msg,
                tg_dest,
                msg_template,
                reactions,
            ...
            tg_msg = self.slave_message_link(
                msg,
                tg_dest,
                msg_template,
                reactions,
            ...
            tg_msg = self.slave_message_sticker(
                msg,
                tg_dest,
                msg_template,
                reactions,
            ...
            if self.flag("send_image_as_file"):
                tg_msg = self.slave_message_file(
                    msg,
                    tg_dest,
                    msg_template,
                    reactions,
                ...
                tg_msg = self.slave_message_image(
                    msg,
                    tg_dest,
                    msg_template,
                    reactions,
            ...
            tg_msg = self.slave_message_animation(
                msg,
                tg_dest,
                msg_template,
                reactions,
            ...
            tg_msg = self.slave_message_voice(
                msg,
                tg_dest,
                msg_template,
                reactions,
            ...
            tg_msg = self.slave_message_location(
                msg,
                tg_dest,
                msg_template,
                reactions,
            ...
            tg_msg = self.slave_message_video(
                msg,
                tg_dest,
                msg_template,
                reactions,
            ...
            tg_msg = self.bot.send_message(
                ...
                text=self._('Unknown type of message "{0}". (UT01)').format(
                    msg.type.name
                ),
            )
    ...
    def html_substitutions(self, msg: Message) -> str:
        ...
        if msg.substitutions:
            ...
            for i in ranges:
                ...
                if isinstance(sub_chat, SelfChatMember) or (
                    isinstance(sub_chat, Chat) and sub_chat.has_self
                ):
    ...
    def slave_message_text(
        ...
        # 链接发给Cubox
        # 稍后看（学习）的TGID为"-824715556"
        if tg_dest == -824715556:
            type = "memo"
            content = text
            send_to_cubox(
                type, content, title=None, description=None, tags=None, folder=None
            )
        ...
        if not old_msg_id:
            ...
            tg_msg = self.bot.edit_message_text(
    ...
    def slave_message_link(
        ...
        # TG个人的TGID为"-212085228"
        # 构造文本消息
        # message_text_test = f"测试：收到一条消息"
        # tg_dest_test = "212085228"  # Replace with the recipient's chat id
        # send_message_test(tg_dest_test, message_text_test)

        # 链接发给Cubox
        # 稍后看（学习）的TGID为"-824715556"
        if tg_dest == -824715556:
            type = "url"
            content = urllib.parse.quote(attributes.url, safe="?=&#:/")
            send_to_cubox(
                type, content, title=None, description=None, tags=None, folder=None
            )
    ...
    def slave_message_image(
        ...
        self.logger.debug(
            "[%s] Message is of %s type; Path: %s; MIME: %s",
            msg.uid,
            msg.type,
            msg.path,
            msg.mime,
        ...
        if msg.path:
            self.logger.debug(
                "[%s] Size of %s is %s.", msg.uid, msg.path, os.stat(msg.path).st_size
            )
        ...
        try:
            ...
            if send_as_file:
                ...
                try:
                    ...
                    self.logger.error(
                        "[%s] Failed to send it as image, sending as document. Reason: %s",
                        msg.uid,
                        e,
                    )
                    assert msg.path
    ...
    def slave_message_animation(
        ...
        self.logger.debug(
            "[%s] Message is an Animation; Path: %s; MIME: %s",
            msg.uid,
            msg.path,
            msg.mime,
        )
        ...
        try:
            ...
            if old_msg_id:
                ...
                assert msg.file and msg.path
                file = self.process_file_obj(msg.file, msg.path)
                file_: Union[IO[bytes], bytes] = open(file, "rb") if isinstance(
                    file, str
                ) else file
                return self.bot.send_animation(
                    tg_dest,
                    InputFile(file_, filename=msg.filename),
                    prefix=msg_template,
                    suffix=reactions,
                    caption=text,
                    parse_mode="HTML",
    ...
    def slave_message_sticker(
        ...
        sticker_reply_markup = self.build_chat_info_inline_keyboard(
            msg, msg_template, reactions, reply_markup
        )
    ...
    @staticmethod
    def build_chat_info_inline_keyboard(
        msg: Message,
        msg_template: str,
        reactions: str,
        ...
        if msg_template:
            description.append(
                [InlineKeyboardButton(msg_template, callback_data="void")]
            )
        if msg.text:
        ...
        sticker_reply_markup.inline_keyboard = (
            description + sticker_reply_markup.inline_keyboard
        )
        return sticker_reply_markup

    def slave_message_file(
        self,
        msg: Message,
        tg_dest: TelegramChatID,
        msg_template: str,
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile() as f:
                pydub.AudioSegment.from_file(msg.file).export(
                    f, format="ogg", codec="libopus", parameters=["-vbr", "on"]
                )
                file = self.process_file_obj(f, f.name)
                tg_msg = self.bot.send_voice(
                    tg_dest,
                    file,
                    prefix=msg_template,
                    suffix=reactions,
    ...
    def slave_message_location(
        ...
        self.logger.debug(
            "[%s] Location message received with old_msg_id %s, compare it with tg_dest %s",
            msg.uid,
            old_msg_id,
            tg_dest,
        )
    ...
    def send_status(self, status: Status):
        if isinstance(status, ChatUpdates):
            ...
            self.logger.debug(
                "Received member updates from channel %s about group %s",
                status.channel,
                status.chat_id,
            )
            for i in status.removed_members:
                self.db.delete_slave_chat_info(
                    status.channel.channel_id, i, status.chat_id
                )
            self.chat_manager.delete_chat_members(
                status.channel.channel_id, status.chat_id, status.removed_members
            )
            chat = status.channel.get_chat(status.chat_id)
            self.chat_manager.update_chat_obj(chat, full_update=True)
        elif isinstance(status, MessageRemoval):
            self.logger.debug(
                "Received message removal request from channel %s on message %s",
            ...
            if old_msg:
                ...
                self.logger.debug(
                    "Found message to delete in Telegram: %s.%s", *old_msg_id
                )
                try:
                ...
                self.logger.info(
                    "Was supposed to delete a message, "
                    "but it does not exist in database: %s",
                    status,
                )
    ...
    def build_reactions_footer(reactions: Reactions) -> str:
        ...
        result = (
            "["
            + ", ".join(f"{k}×{len(v)}" for k, v in reactions.items() if len(v))
            + "]"
        )
        if result == "[]":
            return ""
    ...
    def update_reactions(self, status: MessageReactionsUpdate):
        ...
        old_msg_db = self.db.get_msg_log(
            slave_msg_id=status.msg_id,
            slave_origin_uid=utils.chat_id_to_str(chat=status.chat),
        )
        if old_msg_db is None:
            self.logger.exception(
                "Trying to update reactions of message, but message is not found in database. "
                "Message ID %s from %s, status: %s.",
                status.msg_id,
                status.chat,
                status.reactions,
            )
        ...
        self.dispatch_message(
            old_msg, msg_template, old_msg_id=(chat_id, msg_id), tg_dest=chat_id
        )
    ...
    def generate_message_template(self, msg: Message, singly_linked: bool) -> str:
        ...
        if isinstance(msg.chat, GroupChat):
            self.logger.debug(
                "[%s] Message is from a group. Sender: %s", msg.uid, msg.author
            )
        ...
        if singly_linked:
            ...
            msg_template = (
                f"{Emoji.UNKNOWN} {msg.author.long_name} ({msg.chat.display_name}):"
            )
        return msg_template
    ...
    def check_file_size(self, file: Optional[IO[bytes]]) -> Optional[str]:
        ...
        file.seek(0)
        if (
            not self.channel.flag("local_tdlib_api")
            and file_size > telegram.constants.MAX_FILESIZE_UPLOAD
        ):
            size_str = humanize.naturalsize(file_size)
            max_size_str = humanize.naturalsize(telegram.constants.MAX_FILESIZE_UPLOAD)
            return self._(
                "Attachment is too large ({size}). Maximum allowed by Telegram Bot API is {max_size}. (AT02)"
            ).format(size=size_str, max_size=max_size_str)
        return None

    def process_file_obj(
        self, file: IO[bytes], path: Union[str, Path]
    ) -> Union[IO[bytes], str]:
